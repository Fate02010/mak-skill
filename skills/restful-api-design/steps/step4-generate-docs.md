...
每个 Agent 完成时，同步更新 `WORK_DIR/执行状态.md` 的"Step 4 Agent 状态"表格。
...
- 失败 Agent 数 ≤ 总数 50%：记录失败项，继续合并流程，最后统一补充生成
...
**接口文档内容有效性校验：** 所有成功的临时模块文件，逐一读取并检查：
- 包含至少一个 `###` 三级标题（接口标题）
- 包含 `| **接口地址** |` 行（接口信息表）
- 包含 `#### 请求参数` 和 `#### 响应参数` 章节
- 包含至少一个 JSON 代码块（示例）
...
发现不合格文件 → 记录后重新生成该 Agent 对应的模块文档。
