...
8. Step 6 审视优先对照 `module_brief + page_spec + 任务清单`；只有这些结构化产物无法判定时才回读模块详细文档。
9. 最终产物输出前必须通过 `validate.py` 和 `check_prototype_consistency.py`。
10. 一致性或覆盖率检查失败时，必须回退到 `.prototype-generator/page_specs` 或更上游重建，禁止直接修补最终成品。
11. 任一时刻最多只允许 3 个并行 agent、并行模块任务或并行渲染任务；超过 3 个必须分批。
12. 用户明确授权“自己检查/自己迭代/不要来回交互”后，默认进入自治模式：不再在每轮修复前等待确认，直到真实 `WORK_DIR/prototypes/[产品名称].drawio` 通过门禁或达到熔断轮次。
13. 每轮修复都必须重新生成真实 `.drawio`，并重新执行 `check_prototype_consistency.py`、模块级 `validate.py`、`merge.py`、最终 `validate.py`；只测单页 tmp XML 不算完成。
14. `C13/C10/C11/缺页/低覆盖率/明显功能缺失` 必须回退到 `page_model` 或 `page_spec` 修复；禁止直接改最终 `.drawio` XML 作为主修复方式。
15. 测试、review、对比中间物统一放到 `WORK_DIR/.prototype-generator/`；任务完成后删除无用途的临时文件，禁止泄漏到根目录。
