...
```
检查 WORK_DIR 下是否存在 详细需求文档_overview.md：
  存在 → 分拆模式：
    - 原型图清单来自：详细需求文档_overview.md §7
    - 用户角色/枚举值来自：详细需求文档_overview.md §2 / §5.5
    - 各模块功能细节来自：详细需求文档_[模块中文名].md
    - 记录 DOC_MODE = split，并记录所有模块文件路径列表
  不存在 → 单文件模式：
    - 所有信息来自：详细需求文档.md
    - 记录 DOC_MODE = single
```

> **分拆模式下，后续所有 subagent prompt 中的"需求文档路径"占位符必须替换为正确的文件路径组合（overview + 模块文件），不得填写 `详细需求文档.md`（该文件不存在）。**

1. 读取原型图清单（分拆模式：`详细需求文档_overview.md` §7；单文件模式：`详细需求文档.md` §7）
...
| `[章节名]` | 需求文档中对应章节标题 | 同左 |
| `[风格]` / `[颜色]` | 实际设计风格和主色调 | 同左 |
| `[需求文档读取指令]` | **单文件模式**：`Read [WORK_DIR]/详细需求文档.md 中以下章节`<br>**分拆模式**：`先 Read [WORK_DIR]/详细需求文档_overview.md（获取用户角色 §2 和枚举值字典 §5.5）；再 Read [WORK_DIR]/详细需求文档_[模块中文名].md 中以下章节` | 同左 |
...
2. 判断哪些需要同步回需求文档：
   ...
   - **主动新增**：在原型图清单中补充新增页面
3. **按文档模式回写：**
   - **单文件模式**：编辑 `详细需求文档.md`，末尾追加"变更记录"章节
   - **分拆模式**：功能点变更 → 编辑对应 `详细需求文档_[模块中文名].md`；原型图清单变更（新增页面）→ 编辑 `详细需求文档_overview.md` §7
...
- **单文件模式**：读取 `详细需求文档.md` §7 原型图清单
- **分拆模式**：读取 `详细需求文档_overview.md` §7 原型图清单
...
1. 读取需求文档字段规格（**分拆模式**：按模块逐一读取 `详细需求文档_[模块中文名].md`；**单文件模式**：读取 `详细需求文档.md`），对每个有字段规格的功能点，提取以下信息：
   - 列表页：筛选条件列表 + 列表展示列名列表
