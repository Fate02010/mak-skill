...
| 规则 | 要求 |
|------|------|
| **一个系统一个文件** | 整个系统的所有模块放在同一 `.drawio` 文件的多个 `diagram` 中（单文件多画布模式） |
| **一个模块一个画布** | 每个 `<diagram>` 对应一个功能模块，`name` 为模块名 |
...
```xml
<mxfile host="app.diagrams.net" modified="2024-01-01T00:00:00.000Z" agent="Claude Code" version="24.0.0" type="device">
  <diagram id="aB3dEf7g" name="用户模块">
    <!-- 用户模块：登录页、注册页、个人中心 -->
  </diagram>
  <diagram id="hI5jKl9m" name="订单模块">
    <!-- 订单模块：订单列表、订单详情、下单页 -->
  </diagram>
  <diagram id="nO2pQr6s" name="商品模块">
    <!-- 商品模块：商品列表、商品详情 -->
  </diagram>
</mxfile>
```
...
> 容器内的所有元素 `parent` 指向容器的 `id`，而不是 `"1"`。
...
| 规则 | 检查方式 |
|------|----------|
| **不允许孤立页面** | 每个页面分组必须至少有一条连线（进入或离开），无连线的页面视为孤立页 |
| **不允许无去向主按钮** | 每个主按钮（蓝色填充）必须有 `tooltip` 或对应的 `edge` 连线 |
| **不允许缺失关键业务闭环** | 列表→详情→操作→结果→返回列表的路径必须完整连通 |
| **不允许只画 happy path** | 每个核心页面必须包含空态或异常态的标注，否则视为不完整 |
