...
【关键规则 R7 — CRUD 操作完整性】
含有增删改操作的列表 swimlane，必须在同一 swimlane 或同一 diagram 内体现以下四个元素：
1. **列表操作入口**：列表右上角绘制"新增"主按钮（蓝色，tooltip="→ 新增弹窗/新增页"）；每行操作列绘制"编辑"小按钮（tooltip="→ 编辑弹窗/编辑页"）+ "删除"危险按钮（红色边框，tooltip="→ 删除确认弹窗"）
2. **新增表单**：若为弹窗，在列表 swimlane 的 UI 区右侧绘制 Modal 覆盖层（灰色蒙层 + 白色弹窗卡片 + 表单字段 + 提交/取消按钮）；若为独立页面，单独为其创建 swimlane
3. **编辑表单**：与新增表单结构相同（可复用同一弹窗样式），但标题为"编辑[对象名]"，可在列表 swimlane 旁绘制或与新增弹窗并排说明
4. **删除确认弹窗**：在列表 swimlane UI 区内绘制一个居中 Modal（白色卡片 w=320，标题"确认删除"，正文"确认删除《项目名》？此操作不可撤销"，红色"确认删除"按钮 + 灰色"取消"按钮）；禁止只有文字标注说明"此处弹窗"
