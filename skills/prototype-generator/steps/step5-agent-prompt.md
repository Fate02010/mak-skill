...
第二步：[需求文档读取指令]，逐一提取每个页面的字段列表（字段名 | 类型 | 是否必填）、功能说明、枚举值，以及和页面直接相关的权限、导航、规则展示重点：
  - [页面名称1] → 章节：[### 模块名 > #### 功能点名]
  - [页面名称2] → 章节：[### 模块名 > #### 功能点名]

> [需求文档读取指令] 由主进程替换为以下之一：
> - 单文件模式：读取 [WORK_DIR]/requirements/详细需求文档.md 中以下章节
> - 分拆模式：① 读取 [WORK_DIR]/requirements/index.md 确认本模块详细文档路径和 `requirements/module_briefs/模块摘要_[模块中文名].md` 路径；② 读取 [WORK_DIR]/requirements/详细需求文档_overview.md §2（用户角色）、§2.5（系统边界与外部依赖）、§2.8（模块职责与协作关系）、§3.5/§3.6（导航结构）、§4.5（关键业务事件）、§5.5（枚举值字典）、§5.8（权限与数据口径总则）；③ 先读取模块摘要，提取页面清单、页面 archetype、关键字段索引、状态摘要、CRUD 闭环和跨模块跳转；④ 仅在需要字段明细、复杂校验、状态流转或失败处理时，再读取本模块详细文档中以下章节
...
【CRUD 弹窗/页面清单（必须为每个列表页生成对应的新增/编辑和删除确认界面）】
[CRUD 页面清单]
> 若清单为"无 CRUD 操作"，则跳过本节。否则必须为每个列表页生成：
> 1. 新增/编辑表单（Modal 弹窗或独立页面）— 字段来自需求文档，禁止用序号占位
> 2. 删除确认弹窗 — "确认删除《项目名》？此操作不可撤销" + 红色确认按钮 + 取消按钮
...
> 同时确保 prompt 本身不夹带整份需求文档正文，只保留当前模块需要的章节定位和读取指令。
...
第二步：[需求文档读取指令]，逐一提取每个页面的完整字段列表（字段名 | 类型 | 是否必填）、状态规则、权限限制、导航归属和规则展示重点：
  - [页面名称1] → 章节：[### 模块名 > #### 功能点名]
  - [页面名称2] → 章节：[### 模块名 > #### 功能点名]
...
移动端 x≥400，Web x≥1464。违反则遮挡 UI，UI 临摹无法使用。
标注元素宽度限制（防止溢出相邻 swimlane）：
- 移动端：width ≤ 176（标注区总宽 195，留内边距）
...
[CRUD 页面清单]
> 若清单为"无 CRUD 操作"，则跳过本节。否则必须为每个列表页生成：
> 1. 新增/编辑弹窗 swimlane（套模板七，字段来自需求文档，禁止用序号占位）
> 2. 删除确认弹窗 swimlane（套模板八，含确认文案 + 红色确认按钮 + 取消按钮）
> 弹窗 swimlane 必须计入画布宽度计算。
