...
第二步：Read [WORK_DIR的实际绝对路径]/详细需求文档.md 中以下章节（不要读取整个文档）：
  - [页面名称1] → 章节：[### 模块名 > #### 功能点名]
  - [页面名称2] → 章节：[### 模块名 > #### 功能点名]
第三步：生成前对每个页面做以下强制自检，再开始绘制：
  ① 该页面是登录/注册页？→ 是：无侧边栏、无顶部导航、无面包屑、无筛选区，只有居中卡片+输入框+按钮
  ② 该页面是列表页？→ 筛选区必须是真实 Input/Select/Button 组件，不得画"筛选/操作区"占位框
  ③ 该页面是表单页？→ 字段区必须是真实 Label+Input 逐行排列，不得画"表单区"占位框
  ④ 标注说明必须放在 x ≥ UI区宽+20 的标注区，绝不能出现在 UI 区内
