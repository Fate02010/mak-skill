...
- 允许的推荐格式：`后台-员工管理`、`后台-订单管理`、`小程序-首页`、`App-会员中心`、`H5-活动报名`、`大屏-指挥中心`、`官网门户-产品官网`、`工控机-产线监控`
- **禁止出现中间产物前缀**：`page_spec:`、`page_spec：`、`spec-`、`tmp-`
- **禁止使用系统层笼统名**：`后台管理`、`APP系统`、`小程序端`、`Web端`、`官网门户`、`大屏`、`工控机界面`
...
10. **draw.io 模式交付门槛前置确认（必须执行）**：若 `OUTPUT_FORMAT=drawio`，在阶段 5-3 启动前，主进程必须基于 `原型任务清单.md` 和 `原型DoD.md`，额外生成一份 `WORK_DIR/.prototype-generator/drawio-完成标准.md`，明确“做到什么程度才算完成”。这份文件不是过程说明，而是**交付前验收清单**；后续 Step 5-5 和 Step 6 都必须以它为准。
   ...
   > **强制要求：** draw.io 模式启动前，必须先把这份完成标准落盘。未进入自治模式时，需要展示给用户确认；已进入自治模式时，主进程可直接按此标准继续执行，禁止“先生成再想验收标准”。
...
先判断是否已获自治授权：
- 若用户明确要求“自己检查/自己迭代/不要来回交互/一次生成到位”，则进入自治模式：直接记录执行计划并启动，不再等待本阶段确认
- 否则保留交互模式：列出每个 Agent 负责的模块、页面列表、页面数量，告知输出目录和预计文件数；若为 draw.io 模式，还必须同时展示 `.prototype-generator/原型DoD.md` 与 `.prototype-generator/drawio-完成标准.md` 的摘要，明确“什么状态才算完成”，等待用户确认后再启动

无论哪种模式，都必须把执行计划写入 `WORK_DIR/.prototype-generator/执行状态.md`，并记录：
- 当前模式：interactive / autonomous
- 本轮最大并发：3
- 预计模块数 / page_model 数 / 最终输出路径
- 若为自治模式，本轮最大自迭代轮次：5
...
> draw.io 模式若进入自治模式，主进程必须在阶段 5-5 和 Step 6 之间持续循环：生成真实 `.drawio` → 自动审视 → 定位回退层级 → 修复上游 → 再生成；直到真实产物通过或达到 5 轮熔断。
