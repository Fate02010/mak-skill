...
**目标：** 快速摸清产品资料的整体结构，建立资料导航索引，方便后续按需精准读取。
...
使用 `Glob` 扫描 `WORK_DIR`，匹配以下扩展名：
`*.md, *.html, *.htm, *.pdf, *.docx, *.doc, *.xlsx, *.xls, *.csv, *.pptx, *.ppt, *.png, *.jpg, *.jpeg, *.gif, *.webp, *.zip, *.sql`

对每个文件仅读取文件名和前 20 行做初步类型判断；ZIP 文件用 `unzip -l` 查看内容清单。

### 阶段 1-2：向用户展示文件列表，确认重要性

扫描完成后以表格展示所有文件和 AI 初步判断，询问用户：
...
### 阶段 1-3：分级读取文件，生成 RountMap.md

按用户确认结果处理：
- **核心文件**：完整读取，重点提取实体、关系、业务规则、数据字典
