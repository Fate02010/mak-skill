...
**根据产品需求生成 HTML 交互原型或 draw.io 线框图。**

这是功能最完整的 skill。它会读取产品资料、进行竞品调研、生成详细需求文档，再并行生成各模块的可点击 HTML 原型或 draw.io 图表。
